...
namespace GameProjectReborn.Entities
{
    public class Player : Entity
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            coord = new Vector2((int) Math.Ceiling(Position.X*32/1000) - 1,
                                (int) Math.Ceiling(Position.Y*32/1000) - 1);
            ...
            for (int i = 0; i < spells.Count; i++)
            {
                ...
                if (!KeyboardManager.IsPressed(keys[i]) &&
                    !(MouseManager.IsInRectangle(spellsRect[i]) && MouseManager.IsLeftClicked())) continue;
            ...
            if (KeyboardManager.IsPressed(Keys.Tab) || MouseManager.IsLeftClicked())
            {
                ...
                else
                {
                    ...
                    {
                        if (MouseManager.IsInRectangle(GameScreen.camera.ScreenLocation(MouseManager.Position),
                                                       monster.Bounds))
                            Target = monster;
                    }
                }
